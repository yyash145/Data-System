 ...
 * @return false 
 */
void BufferManager::deleteFromPool(string tableName, int pageIndex)
{   
...
void BufferManager::deleteAll()
{   
    logger.log("BufferManager::deleteAll");
    this->pages.clear();
}
...
void BufferManager::renameFile(string oldMatrixName, string newMatrixName){
    logger.log("BufferManager::renameFile");
    Table *table = tableCatalogue.getTable(newMatrixName);
    uint bc = table->blockCount;
    // logger.log("blockCount" + to_string(bc));
    for(auto i=0; i<bc; i++){
        string oldPageName = "../data/temp/" + oldMatrixName + "_Page" + to_string(i);
