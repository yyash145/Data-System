...
void TableCatalogue::changeMatrixName(string oldMatrixName, string newMatrixName)
{
    logger.log("TableCatalogue::changeMatrixName");

    for(auto it: tables){
        if(it.first == oldMatrixName){
            logger.log(it.first + " " + newMatrixName);
            tables[newMatrixName] = it.second;
            tables[newMatrixName]->isMatrix = true;
